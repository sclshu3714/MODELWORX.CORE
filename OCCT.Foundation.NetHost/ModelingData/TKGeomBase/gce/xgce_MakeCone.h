...
namespace TKGeomBase {
	...
	public ref class xgce_MakeCone : public xgce_Root
	{
	public:
		...
		virtual gce_MakeCone* GetMakeCone();
		...
		xgce_MakeCone(xgp_Ax2^ A2, Standard_Real Ang, Standard_Real Radius);

		//! Makes a Cone from gp <TheCone> coaxial to another
		//! Cone <Cone> and passing through a Pnt <Point>.
		xgce_MakeCone(xgp_Cone^ Cone, xgp_Pnt^ Point);

		//! Makes a Cone from gp <TheCone> coaxial to another
		//! Cone <Cone> at the distance <Dist> which can
		//! be greater or lower than zero.
		xgce_MakeCone(xgp_Cone^ Cone, Standard_Real Dist);
		...
		//! "NullAngle".
		xgce_MakeCone(xgp_Pnt^ P1, xgp_Pnt^ P2, xgp_Pnt^ P3, xgp_Pnt^ P4);
		...
		//! "ConfusedPoints"
		xgce_MakeCone(xgp_Ax1^ Axis, xgp_Pnt^ P1, xgp_Pnt^ P2);
		...
		//! If <P1> and <P2> are confused we have the status
		//! "ConfusedPoints"
		xgce_MakeCone(xgp_Lin^ Axis, xgp_Pnt^ P1, xgp_Pnt^ P2);
		...
		xgce_MakeCone(xgp_Pnt^ P1, xgp_Pnt^ P2, Standard_Real R1, Standard_Real R2);
		...
		/// ±¾µØ¾ä±ú
		/// </summary>
		property gce_MakeCone* IHandle {
			gce_MakeCone* get() {
				return 	NativeHandle;
			}
			void set(gce_MakeCone* handle) {
				NativeHandle = handle;
			}
		}

	private:
		gce_MakeCone* NativeHandle;
	};
}
