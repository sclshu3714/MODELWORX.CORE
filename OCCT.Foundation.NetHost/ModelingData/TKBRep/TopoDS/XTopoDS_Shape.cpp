...
namespace TKBRep {
    ...
    template<class T2>
    XTopoDS_Shape::XTopoDS_Shape(T2^ theOther, typename std::enable_if<opencascade::std::is_base_of<XTopoDS_Shape, T2>::value>::type*) {
    ...
    TopAbs_Orientation XTopoDS_Shape::Orientation() {
        return NativeHandle->Orientation();
    };

    //! Sets the shape orientation.
    void XTopoDS_Shape::Orientation(TopAbs_Orientation theOrient) {
        NativeHandle->Orientation(theOrient);
    };

    //! Returns  a    shape  similar  to  <me>   with  the
    //! orientation set to <Or>.
    XTopoDS_Shape^ XTopoDS_Shape::Oriented(TopAbs_Orientation theOrient) {
        return gcnew XTopoDS_Shape(NativeHandle->Oriented(theOrient));
    };
    ...
    TopAbs_ShapeEnum XTopoDS_Shape::ShapeType() {
        return NativeHandle->ShapeType();
    };
    ...
    void XTopoDS_Shape::Compose(TopAbs_Orientation theOrient) {
        NativeHandle->Compose(theOrient);
    };

    //! Returns  a  shape   similar   to  <me>   with  the
    //! orientation composed with theOrient, using the
    //! Compose method from the TopAbs package.
    XTopoDS_Shape^ XTopoDS_Shape::Composed(TopAbs_Orientation theOrient) {
        return gcnew XTopoDS_Shape(NativeHandle->Composed(theOrient));
    };
