...
#ifndef _XTDocStd_Application_HeaderFile
#define _XTDocStd_Application_HeaderFile
#pragma once
...
namespace TKLCAF {
    ...
    public:
        ...
        virtual Handle(Resource_Manager) Resources();
        ...
        void DefineFormat(const TCollection_AsciiString& theFormat, TCollection_AsciiString& theDescription, TCollection_AsciiString& theExtension, Handle(PCDM_RetrievalDriver)& theReader, Handle(PCDM_StorageDriver)& theWriter);
        ...
        void GetDocument(const Standard_Integer index, Handle(TDocStd_Document)& aDoc);
        ...
        //! for (Standard_Integer i = 1; i <= nbdoc; i++) {
        //! aApp->GetDocument(i,aDoc);
        void GetDocument(const Standard_Integer index, XTDocStd_Document^ aDoc);
        ...
        //! application, the new document is handled by the
        //! applicative session.
        virtual void NewDocument(XTCollection_ExtendedString^ format, XTDocStd_Document^ aDoc);
        ...
        //! application, the new document is handled by the
        //! applicative session.
        virtual void NewDocument(System::String^ format, XTDocStd_Document^ aDoc);
        ...
        //! =======================
        virtual void InitDocument(const Handle(TDocStd_Document)& aDoc);
        ...
        //! to open/save a document
        //! =======================
        virtual void InitDocument(XTDocStd_Document^ aDoc);
        ...
        //! Close the given document. the document is not any more
        //! handled by the applicative session.
        void Close(XTDocStd_Document^ aDoc);
        ...
        Standard_Integer IsInSession(XTCollection_ExtendedString^ path);
        ...
        PCDM_ReaderStatus Open(XTCollection_ExtendedString^ path, XTDocStd_Document^ aDoc);
        ...
        PCDM_ReaderStatus Open(Standard_IStream& theIStream, XTDocStd_Document^ theDoc);

        //! Save the  active document  in the file  <name> in the
        //! path <path> ; o verwrites  the file  if  it already exists.
        PCDM_StoreStatus SaveAs(const Handle(TDocStd_Document)& aDoc, TCollection_ExtendedString& path);
        //! Save the  active document  in the file  <name> in the
        //! path <path> ; o verwrites  the file  if  it already exists.
        PCDM_StoreStatus SaveAs(XTDocStd_Document^ aDoc, XTCollection_ExtendedString^ path);
        ...
        //! Save theDoc to standard SEEKABLE stream theOStream.
        //! the stream should support SEEK fuctionality
        PCDM_StoreStatus SaveAs(XTDocStd_Document^ theDoc, Standard_OStream& theOStream);
        ...
        //! Standard_NotImplemented if the document
        //! was not retrieved in the applicative session by using Open.
        PCDM_StoreStatus Save(XTDocStd_Document^ aDoc);

        //! Save the  active document  in the file  <name> in the
        //! path <path>  .  overwrite  the file  if  it
        //! already exist.
        PCDM_StoreStatus SaveAs(const Handle(TDocStd_Document)& aDoc, TCollection_ExtendedString& path, TCollection_ExtendedString& theStatusMessage);

        //! Save the  active document  in the file  <name> in the
        //! path <path>  .  overwrite  the file  if  it
        //! already exist.
        PCDM_StoreStatus SaveAs(XTDocStd_Document^ aDoc, XTCollection_ExtendedString^ path, XTCollection_ExtendedString^ theStatusMessage);
        ...
        //! Save theDoc TO standard SEEKABLE stream theOStream.
        //! the stream should support SEEK fuctionality
        PCDM_StoreStatus SaveAs(XTDocStd_Document^ theDoc, Standard_OStream& theOStream, XTCollection_ExtendedString^ theStatusMessage);
        ...
        //! Save the document overwriting the previous file
        PCDM_StoreStatus Save(XTDocStd_Document^ aDoc, XTCollection_ExtendedString^ theStatusMessage);
        ...
        //! Notification that is fired at each OpenTransaction event.
        virtual void OnOpenTransaction(XTDocStd_Document^ theDoc);
        ...
        //! Notification that is fired at each CommitTransaction event.
        virtual void OnCommitTransaction(XTDocStd_Document^ theDoc);
        ...
        //! Notification that is fired at each AbortTransaction event.
        virtual void OnAbortTransaction(XTDocStd_Document^ theDoc);
    ...
    private:
        NCollection_Haft<Handle(TDocStd_Application)> NativeHandle;
    };
}
#endif // _XTDocStd_Application_HeaderFile
